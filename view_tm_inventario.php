...
?>
    ...
    <div class="container">
        <h2>Lista de inventario</h2><br>

        <!-- Formulario para agregar -->
        <h3>Agregar reciclaje al inventario</h3>
...
<div class="mb-3">
    <select class="form-control" name="tip_mate_codg" id="tip_mate_codg" required>
        ...
        <?php
        // Obtén el valor seleccionado del primer select
        $selectedMaterial = isset($_POST['mate_codg']) ? $_POST['mate_codg'] : '';
        echo "Material Seleccionado: " . $selectedMaterial; // Agregamos esto para depuración

        $statement = $con->prepare('SELECT * FROM tr_tip_mat WHERE mate_codg = ?');
        $statement->execute([$selectedMaterial]);

        // Imprimimos el número de filas devueltas para depuración
        echo "Número de filas devueltas: " . $statement->rowCount();
        ...
        <h3>Editar Roles</h3>
    <?php
        $insert = $con->prepare ('SELECT * FROM tm_inventario INNER JOIN tr_tip_mat ON tm_inventario.inv_codg = tr_tip_mat.tip_mate_codg');
        $insert->execute();
